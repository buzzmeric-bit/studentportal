...
class CoreDataService {
  ...
  /// Live semesters for academic year (realtime + periodic poll fallback)
  Stream<List<SemesterInfo>> watchSemestersForAcademicYear(String academicYearId) {
    ...
    final poll = Stream.periodic(const Duration(seconds: 30))
        .asyncMap((_) => getSemestersForAcademicYear(academicYearId));

    return Stream.multi((controller) {
      final subRealtime = realtime.listen(controller.add, onError: controller.addError);
      final subPoll = poll.listen(controller.add, onError: controller.addError);

      // Push an immediate snapshot so UI has data even before realtime fires.
      getSemestersForAcademicYear(academicYearId)
          .then(controller.add)
